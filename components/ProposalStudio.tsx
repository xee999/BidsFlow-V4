...
const ProposalStudio: React.FC<ProposalStudioProps> = ({ bids, onUpdateBid, vaultAssets }) => {
  ...
  const handleAIGenerateSection = async () => {
    if (!selectedBid || !activeSection) return;
    setIsGenerating(true);
    const docContext = bidDocuments.map(d => `${d.name}: ${d.summary || d.category}`).join('\n');
    const draft = await draftProposalSection(activeSection.title, (activeSection as any).description || 'Professional narrative', selectedBid, docContext);
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          .cover { height: 260mm; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; border: 15px solid #D32F2F; margin: -10mm; padding: 20mm; }
          .jazz-logo { color: #D32F2F; font-weight: 800; font-size: 64px; border: 10px solid #D32F2F; padding: 20px 40px; margin-bottom: 80px; }
          h1 { font-size: 56px; font-weight: 800; color: #D32F2F; text-transform: uppercase; margin-bottom: 30px; letter-spacing: -3px; line-height: 1; }
          .client { font-size: 28px; font-weight: 800; color: #1e3a5f; margin-bottom: 100px; }
          .chapter-header { font-size: 28px; color: #D32F2F; border-bottom: 5px solid #D32F2F; padding-bottom: 15px; margin-top: 60px; text-transform: uppercase; font-weight: 800; }
          p { margin-bottom: 25px; text-align: justify; font-size: 16px; color: #334155; }
          strong { color: #000; font-weight: 800; }
          table { width: 100%; border-collapse: collapse; margin: 20px 0; }
          td, th { border: 1px solid #e2e8f0; padding: 12px; text-align: left; }
          th { background: #f8fafc; font-weight: 800; text-transform: uppercase; font-size: 11px; }
          .footer { position: absolute; bottom: 20mm; width: 100%; text-align: center; font-size: 11px; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 2px; }
        </style>
      </head>
      <body>
        <div class="canvas">
          <div class="cover page-break">
            ...
            <div style="font-size:16px; font-weight:800; color:#64748b; text-transform:uppercase; letter-spacing:8px; margin-bottom:20px;">Technical Response</div>
            <h1>${selectedBid.projectName}</h1>
            <div class="client">Prepared for ${selectedBid.customerName}</div>
            <div class="footer">CONFIDENTIAL | PMCL GOVERNANCE</div>
          </div>
          ${sections.map((s, i) => `
            <div class="page-break">
              <div style="color:#94a3b8; font-weight:800; font-size:12px;">SECTION 0${i + 1}</div>
              <h2 class="chapter-header">${s.title}</h2>
              <div style="margin-top: 50px;">
                ${s.content.split('\n').map(l => {
                  let line = l.trim().replace(/\*\*/g, '');
                  if (line.startsWith('### ')) return `<h3 style="color:#1e3a5f; font-size:22px; font-weight:800; margin-top:40px; text-transform:uppercase;">${line.replace(/^###\s*/, '')}</h3>`;
                  if (line.startsWith('#### ')) return `<h4 style="font-size:18px; font-weight:700; margin-top:30px;">${line.replace(/^####\s*/, '')}</h4>`;
                  if (line.startsWith('- ') || line.startsWith('* ')) return `<div style="margin-left:30px; margin-bottom:15px; font-size:16px;">• ${line.replace(/^[\-\*]\s*/, '')}</div>`;
                  if (line.startsWith('|')) {
